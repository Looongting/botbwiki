...
### 1. AI Prompt配置
机器人支持为AI对话自动添加prompt前缀，可以通过环境变量配置：

```bash
# AI Prompt前缀（可选，自动添加到用户问题前）
AI_PROMPT_PREFIX=请直接回答用户问题，不要使用markdown语法，回复要简洁明了。用户问题：
```

**配置说明**：
- 当用户使用 `?ai` 询问时，系统会自动在用户问题前添加配置的prompt
- 默认prompt：`请直接回答用户问题，不要使用markdown语法，回复要简洁明了。用户问题：`
- 可以根据需要自定义prompt，例如：
  - `请用中文回答，不要使用markdown格式：`
  - `请简洁回答，避免使用代码块和特殊格式：`
  - `请直接回答，不要使用任何格式化语法：`
